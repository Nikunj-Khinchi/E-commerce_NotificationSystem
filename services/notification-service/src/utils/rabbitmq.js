...
const connect = async () => {
    try {
        ...
        await channel.assertExchange(config.rabbitmq.exchanges.notification, 'topic', { durable: true });
        ...
        await channel.assertQueue(config.rabbitmq.queues.promotions, { durable: true });
        await channel.assertQueue(config.rabbitmq.queues.orderUpdates, { durable: true });
        await channel.assertQueue(config.rabbitmq.queues.recommendations, { durable: true });
